// 出力先テンプレート。書き直しと文脈整形のプロンプトに制約を足し、
// 生成後の決定論的サニタイズ(utils/sanitize.js)と辞書プリセットを紐づける。
// 文言の実体はここに集約する。TextEditor側にリテラルを置かない。
...
- 句読点で改行しない。段落は1行に繋げる
- 日本語と半角英数字のあいだにスペースを入れない
- 絵文字を使わない
- 生成ツールの署名を入れない(Claude関連の署名、Co-Authored-By、Generated with等は一切不要)`;
...
export const OUTPUT_TARGETS = [
  ...
  },
  {
    ...
    stripSignatures: true,
    preset: 'business',
    label: { ja: 'PR本文', en: 'PR description' },
    desc: { ja: 'GitHubのプルリクエスト本文。Markdown可', en: 'GitHub pull request description. Markdown allowed' },
    instruction: {
      ja: `${SHARED_JA}
...
- Markdownの見出しと箇条書きは使ってよい。ただし構造を作るためだけの空の節を置かない
...
- Markdown headings and lists are allowed, but do not add empty sections just to create structure`,
  ...
  },
  {
    ...
    desc: { ja: 'GitHubのissue本文。課題と次の行動を明確にする', en: 'GitHub issue. State the problem and the next action' },
    instruction: {
      ja: `${SHARED_JA}
...
- Describe facts, not fault`,
  ...
  },
  {
    ...
    desc: { ja: '1行目に要約、空行、本文。署名なし', en: 'Summary line, blank line, body. No sign-off' },
    instruction: {
      ja: `${SHARED_JA}

【出力先: コミットメッセージ】
- 1行目は変更の要約。命令形か体言止めで、50文字程度に収める。末尾に句点を打たない
- 2行目は空行にする
- 3行目以降に、なぜその変更をしたのかを書く。何をしたかはコードを見れば分かるので、理由を優先する
- 本文が不要なほど自明な変更なら、1行目だけで終える
...
- Avoid heavy bold and headings; this comment is published under the reviewer's own name`,
  ...
  },
  {
    ...
    stripSignatures: true,
    preset: 'business',
    label: { ja: 'Slackレビュー依頼', en: 'Slack review request' },
    desc: { ja: 'プレーンテキスト。URLはフルパスで書く', en: 'Plain text. URLs written in full' },
    instruction: {
      ja: `${SHARED_JA}

【出力先: Slackのレビュー依頼】
- プレーンテキストで書く。Markdown記法を使わない。見出し記法、太字、リンク記法を使わない
- URLは短縮せずフルパスで書く。リンクにラベルを付けず、URLをそのまま置く
- 冒頭で何のレビューを頼んでいるかが分かるようにする
- 変更の要点を2行から4行で書く。差分の全項目を書き写さない
