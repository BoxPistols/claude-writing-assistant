// ux-writing-dead-clicheの配布物をsrc/vendor/ に取り込む。
// 検出ロジックはThe-Writeでは書かない。上流をそのまま複製し、差分が出たらここで検知する。
...
for (const t of TARGETS) {
  ...
  // JSONのままだとNodeがimport attributeを要求しViteと挙動が割れるので、JSモジュールとして吐く。
...
// 取り込んだ結果がThe-Writeの想定と噛み合うかを、その場で実測して出す。
...
if (typeof engine.rulesForPresetData !== 'function') {
  throw new Error('上流のengineにrulesForPresetDataがない。取り込みを中止する。');
}
const manual = data.rules.filter((r) => r.manual === true).length;
console.log(`\n辞書v${data.version}: 規則${data.rules.length}件(自動${data.rules.length - manual} / 手動${manual})`);
...
console.log(ov.length ? `\nseverity上書きあり: ${ov.map(([k, v]) => `${k}(${Object.keys(v.severity).length}件)`).join(', ')}`
                      : '\n警告: severity上書きが1件もない。プリセット間で差が出ない可能性がある。');
console.log(changed ? `\n${changed}件更新した。` : '\nすべて最新。');
